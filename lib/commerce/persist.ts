...
import { getCommerceDatabase } from "./database";
import { persistTrustedCommerceAttemptInTransaction } from "./persistence-core";
...
export function persistCommercialAttempt(request: CreateCommerceAttemptRequest) {
  return getCommerceDatabase().begin(async (transaction) => {
